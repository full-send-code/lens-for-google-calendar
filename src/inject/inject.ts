...
const parseShortcutText = (text: string): ShortcutText => {
  return {
...
function loadGroups(): void {
  try {
    chrome.storage.sync.get("groups", (items: any) => {
...
function migrateToV1(groups: any): any {
  const name2id = (calName: string) => {
    const cal = (window as any).CalendarManager.calendars.byName[calName];
    if (cal) {
      return cal.id;
    } else {
  ...
  const v1Groups: any = {};
  for (const groupName of Object.keys((window as any).CalendarManager.exportGroups(false, groups))) {
